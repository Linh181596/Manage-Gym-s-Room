...
public class ScheduleSetupController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (reg == null) {
                if (currentUser.getRole() == com.mycompany.gymcentermanagement.model.entity.User.Role.PT) {
            ...
            if (currentUser.getRole() == com.mycompany.gymcentermanagement.model.entity.User.Role.PT) {
                com.mycompany.gymcentermanagement.service.PersonalTrainerService personalTrainerService = 
                        new com.mycompany.gymcentermanagement.service.impl.PersonalTrainerServiceImpl();
                com.mycompany.gymcentermanagement.model.entity.PersonalTrainer pt = 
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (daysOfWeekStr == null || daysOfWeekStr.length == 0) {
                sendBackError(request, response, regId, ptId, memberId, "Vui lòng chọn ít nhất một ngày.", actualStartDate, daysOfWeekStr, timeSlot);
                return;
            }

            // Kiểm tra giới hạn thời gian (không quá 1 năm trong tương lai)
            if (actualStartDate.isAfter(LocalDate.now().plusYears(1))) {
                sendBackError(request, response, regId, ptId, memberId, "Ngày bắt đầu chính thức không được vượt quá 1 năm trong tương lai.", actualStartDate, daysOfWeekStr, timeSlot);
                return;
            }

            String result = ptScheduleService.generateFixedScheduleForPT(
                    regId, ptId, actualStartDate, Arrays.asList(daysOfWeekStr), timeSlot, currentUser.getUserId());
            ...
            if ("SUCCESS".equals(result)) {
                ...
                sendBackError(request, response, regId, ptId, memberId, result, actualStartDate, daysOfWeekStr, timeSlot);
            }
    ...
    private void sendBackError(HttpServletRequest request, HttpServletResponse response,
                               ...
                               LocalDate actualStartDate, String[] daysOfWeekStr, String timeSlot)
            throws ServletException, IOException {
        request.setAttribute("errorMsg", errorMsg);
        request.setAttribute("submittedDays", daysOfWeekStr != null ? Arrays.asList(daysOfWeekStr) : new ArrayList<String>());
