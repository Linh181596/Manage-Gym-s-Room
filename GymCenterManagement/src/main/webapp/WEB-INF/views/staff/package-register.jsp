...
<div class="container-fluid pt-4 px-4">
    ...
    <div class="mb-4">
        <div>
            <h4 class="mb-0 text-dark fw-bold"><i class="fa fa-user-plus me-2 text-primary"></i>Đăng ký gói tập Gym</h4>
            <small class="text-muted">Đăng ký gói tập luyện và gói thẻ thành viên cho hội viên phòng tập</small>
        </div>
    </div>
    ...
    <div class="row g-4">
        ...
        <div class="col-xl-7 col-lg-8">
            <div class="bg-light rounded p-4 p-md-5 shadow-sm border-0 h-100">
                <h5 class="text-dark fw-bold mb-4 border-bottom pb-2">Chọn thông tin đăng ký</h5>
                ...
                <form action="${pageContext.request.contextPath}/staff/register-package" method="post" id="registrationForm" class="needs-validation" novalidate>
                    ...
                    <div class="mb-4">
                        <label for="memberSearch" class="form-label fw-bold text-dark"><i class="fa fa-user me-1 text-muted"></i> 1. Tìm kiếm và chọn hội viên <span class="text-danger">*</span></label>
                        <div class="input-group mb-2">
                            <span class="input-group-text bg-white text-muted border-end-0"><i class="fa fa-search"></i></span>
                            <input type="text" id="memberSearch" class="form-control border-start-0" placeholder="Nhập tên, email hoặc số điện thoại hội viên...">
                        </div>
                        <select class="form-select form-select-lg border-2" id="memberSelect" name="memberId" size="5" style="height: 140px; overflow-y: auto;" required>
                            <c:forEach var="member" items="${members}">
                                <option value="${member.memberId}" 
                                    ...
                                    ${member.userDetails.fullName} (Mã: MEM-${member.memberId} | SĐT: ${member.userDetails.phoneNumber})
                                </option>
                            </c:forEach>
                        </select>
                        <div class="invalid-feedback">Vui lòng chọn một hội viên.</div>
                        <div class="form-text text-muted">Cuộn và chọn hoặc nhấp đúp vào hội viên tương ứng từ danh sách.</div>
                    </div>

                    <!-- Package Selector -->
                    <div class="mb-5">
                        <label for="packageSelect" class="form-label fw-bold text-dark"><i class="fa fa-box me-1 text-muted"></i> 2. Chọn gói tập Gym <span class="text-danger">*</span></label>
        ...
        <div class="col-xl-5 col-lg-4">
            <div class="card border-0 shadow-sm rounded h-100 bg-white">
                <div class="card-header bg-dark text-white p-4 border-0 rounded-top d-flex align-items-center">
                    ...
                    <h5 class="mb-0 fw-bold">Tóm tắt đăng ký</h5>
                </div>
                <div class="card-body p-4 d-flex flex-column justify-content-between">
                    <div>
                        ...
                        <div class="mb-4">
                            ...
                            <div id="summaryMemberContainer" class="p-3 bg-light rounded border border-dashed text-muted">
                                <i class="fa fa-user-circle me-1"></i> Chưa có hội viên nào được chọn.
                            </div>
                        </div>
                    ...
                    <div class="border-top pt-4 mt-auto">
                        ...
                        <div class="alert alert-warning small border-0 py-2 px-3 mb-0" role="alert">
                            <i class="fa fa-info-circle me-1"></i> Gửi biểu mẫu này sẽ tạo một <strong>Hóa đơn Chờ thanh toán</strong> và đăng ký gói tập với trạng thái <strong>Chờ xử lý</strong>.
                        </div>
                    </div>
                </div>
            </div>
        ...
        memberSelect.addEventListener("change", function() {
            ...
            if (selectedOpt) {
                ...
                summaryMember.innerHTML = `
                    ...
                    <div class="small text-muted"><i class="fa fa-id-card me-1"></i> Mã: MEM-\${id}</div>
